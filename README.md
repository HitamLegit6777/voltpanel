...
VoltPanel's codebase is indexed with [Graphify](https://github.com/Graphify-Labs/graphify), producing a local AST-derived knowledge graph with **1,577 nodes**, **5,128 edges**, and **47 communities**. The current snapshot was generated from commit `8654eac` without LLM extraction.

```mermaid
flowchart LR
    UI[Pulse UI<br/>Vanilla JavaScript] --> API[Axum API]
    API --> AUTH[Identity and capabilities]
    API --> STATE[AppState]
    STATE --> DB[(SQLite / Db)]
    STATE --> PROC[ProcManager]
    STATE --> NODE[NodeClient]
    STATE --> CONSOLE[ConsoleHub]
    STATE --> FLOW[Scheduler / Flows]
    PROC --> LOCAL[Local native workloads]
    NODE --> VOLTD[Remote voltd agents]
    VOLTD --> SANDBOX[Namespaces + cgroup v2]
```

[![VoltPanel Graphify knowledge graph](docs/architecture/graph-preview.png)](docs/architecture/graph.html)

The central architecture hubs are `AppState`, `Db`, `AuthUser`, `Server`, `DaemonRuntime`, `Node`, `ProcManager`, and `NodeClient`.

| View | Purpose |
|---|---|
| [Interactive knowledge graph](docs/architecture/graph.html) | Search nodes, filter communities, and inspect code relationships |
| [Architecture tree](docs/architecture/graph-tree.html) | Browse the repository hierarchy and symbol counts |
| [Call-flow map](docs/architecture/callflow.html) | Explore generated call relationships |
| [Graph report](docs/architecture/GRAPH_REPORT.md) | Read hubs, communities, inferred edges, and import-cycle analysis |

Regenerate locally after structural changes:

```bash
graphify extract . --code-only --cargo --no-label
graphify cluster-only . --no-label
graphify tree --root . --label VoltPanel
