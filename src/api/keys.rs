...
pub async fn create(State(state): State<AppState>, AuthUser(u): AuthUser, Json(req): Json<CreateReq>) -> ApiResult<Json<serde_json::Value>> {
    ...
    Ok(Json(json!({ "id": id, "token": raw, "name": req.name, "scopes": scopes })))
}
...
pub async fn delete(State(state): State<AppState>, AuthUser(u): AuthUser, Path(id): Path<i64>) -> ApiResult<Json<serde_json::Value>> {
    ...
    let owner: Option<i64> = conn
        .query_row("SELECT user_id FROM api_keys WHERE id=?1", [id], |r| r.get(0))
        .ok();
    drop(conn);
