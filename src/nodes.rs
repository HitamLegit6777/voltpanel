...
impl Node {
    pub fn online(&self) -> bool {
        if !self.enabled || !self.enrolled { return false; }
        self.last_heartbeat
            .as_deref()
    ...
    pub fn available_memory_mb(&self) -> i64 {
        let hard = if self.memory_limit_mb > 0 { self.memory_limit_mb } else { (self.capacity.memory_total / 1_048_576) as i64 };
        let allowed = hard.saturating_mul(100 + self.memory_overallocate).saturating_div(100);
        allowed.saturating_sub((self.capacity.memory_used / 1_048_576) as i64)
    }

    pub fn available_disk_mb(&self) -> i64 {
        let hard = if self.disk_limit_mb > 0 { self.disk_limit_mb } else { (self.capacity.disk_total / 1_048_576) as i64 };
        let allowed = hard.saturating_mul(100 + self.disk_overallocate).saturating_div(100);
...
fn from_row(r: &Row) -> rusqlite::Result<Node> {
    Ok(Node {
        id: r.get(0)?, uuid: r.get(1)?, name: r.get(2)?, public_url: r.get(3)?, secret: r.get(4)?, enrollment_token: r.get(5)?,
        enrolled: r.get::<_, i64>(6)? != 0, enabled: r.get::<_, i64>(7)? != 0, maintenance: r.get::<_, i64>(8)? != 0,
        schedulable: r.get::<_, i64>(9)? != 0, location: r.get(10)?,
        tags: serde_json::from_str(&r.get::<_, String>(11)?).unwrap_or_default(),
        memory_limit_mb: r.get(12)?, disk_limit_mb: r.get(13)?, cpu_limit_percent: r.get(14)?,
        memory_overallocate: r.get(15)?, disk_overallocate: r.get(16)?, daemon_version: r.get(17)?, hostname: r.get(18)?,
        os: r.get(19)?, arch: r.get(20)?, capacity: serde_json::from_str(&r.get::<_, String>(21)?).unwrap_or_default(),
        last_heartbeat: r.get(22)?, last_error: r.get(23)?, created_at: r.get(24)?, updated_at: r.get(25)?,
    })
}
...
pub fn create(db: &Db, name: &str, public_url: &str, location: &str, tags: &[String]) -> Result<Node> {
    ...
    conn.query_row(&format!("SELECT {COLS} FROM nodes WHERE id=?1"), [id], from_row).map_err(Into::into)
}
...
pub fn get(db: &Db, id: i64) -> Result<Node> {
    let conn = db.lock();
    conn.query_row(&format!("SELECT {COLS} FROM nodes WHERE id=?1"), [id], from_row).context("node not found")
}

pub fn get_by_uuid(db: &Db, uuid: &str) -> Result<Node> {
    let conn = db.lock();
    conn.query_row(&format!("SELECT {COLS} FROM nodes WHERE uuid=?1"), [uuid], from_row).context("node not found")
}

pub fn get_by_name(db: &Db, name: &str) -> Result<Node> {
    let conn = db.lock();
    conn.query_row(&format!("SELECT {COLS} FROM nodes WHERE name=?1"), [name], from_row).context("node not found")
}

pub fn enroll(db: &Db, token: &str, heartbeat: &NodeHeartbeat) -> Result<Node> {
    let conn = db.lock();
    let id: Option<i64> = conn.query_row("SELECT id FROM nodes WHERE enrollment_token=?1 AND enrolled=0", [token], |r| r.get(0)).optional()?;
...
pub fn heartbeat(db: &Db, uuid: &str, heartbeat: &NodeHeartbeat) -> Result<()> {
    ...
    if changed == 0 { bail!("node not enrolled"); }
    Ok(())
}

#[allow(clippy::too_many_arguments)]
pub fn update(
    db: &Db, id: i64, name: &str, public_url: &str, enabled: bool, maintenance: bool, schedulable: bool,
    location: &str, tags: &[String], memory_limit_mb: i64, disk_limit_mb: i64,
    memory_overallocate: i64, disk_overallocate: i64,
) -> Result<Node> {
    let conn = db.lock();
    conn.execute(
...
pub fn rotate_secret(db: &Db, id: i64) -> Result<String> {
    let secret = crate::auth::random_token(48);
    let conn = db.lock();
    conn.execute("UPDATE nodes SET secret=?1,updated_at=?2 WHERE id=?3", params![secret, now(), id])?;
    Ok(secret)
}
...
pub fn set_error(db: &Db, uuid: &str, error: &str) -> Result<()> {
    let conn = db.lock();
    conn.execute("UPDATE nodes SET last_error=?1,updated_at=?2 WHERE uuid=?3", params![error, now(), uuid])?;
    Ok(())
}
...
pub fn delete(db: &Db, id: i64) -> Result<()> {
    ...
    if assigned > 0 { bail!("node has {assigned} assigned server(s)"); }
    conn.execute("DELETE FROM nodes WHERE id=?1", [id])?;
    Ok(())
}

pub fn record_event(db: &Db, node_id: i64, level: &str, kind: &str, message: &str, payload: &serde_json::Value) -> Result<()> {
    let conn = db.lock();
    conn.execute(
...
pub fn select_for_server(db: &Db, memory_mb: i64, disk_mb: i64, required_tags: &[String], location: Option<&str>) -> Result<Node> {
    let mut candidates: Vec<Node> = list(db)?.into_iter().filter(|n| {
        n.online() && n.enabled && n.schedulable && !n.maintenance &&
        location.map(|loc| loc.is_empty() || n.location == loc).unwrap_or(true) &&
        required_tags.iter().all(|tag| n.tags.iter().any(|t| t == tag)) &&
        n.available_memory_mb() >= memory_mb && n.available_disk_mb() >= disk_mb
    }).collect();
    candidates.sort_by(|a, b| {
        let a_score = a.capacity.cpu_percent + (a.capacity.memory_used as f64 / a.capacity.memory_total.max(1) as f64 * 100.0) + a.capacity.servers_running as f64 * 2.0;
        let b_score = b.capacity.cpu_percent + (b.capacity.memory_used as f64 / b.capacity.memory_total.max(1) as f64 * 100.0) + b.capacity.servers_running as f64 * 2.0;
        a_score.total_cmp(&b_score)
    });
    candidates.into_iter().next().context("no online node has enough capacity")
}
...
pub fn port_available_on_node(db: &Db, node: &str, port: i64) -> Result<bool> {
    ...
    let used: i64 = conn.query_row(
        ...
        params![node, port], |r| r.get(0),
    )?;
    Ok(used == 0)
}

pub fn next_free_port_on_node(db: &Db, node: &str, range_start: i64, range_end: i64) -> Result<i64> {
    for port in range_start..=range_end {
        if port_available_on_node(db, node, port)? { return Ok(port); }
    }
